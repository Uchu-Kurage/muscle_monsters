...
// 種目で使う器具の種類。「自重かどうか(isBodyweight)」とは別軸で扱う
// （例: 懸垂は自重だが鉄棒が必要なので equipment='bar'）。
type EquipmentType = 'barbell' | 'dumbbell' | 'machine' | 'bar' | 'roller' | 'none';

// 器具バッジの表示メタデータ（ラベル・アイコン・色）。'none' は器具なし（自重のみ）。
const EQUIPMENT_INFO: Record<EquipmentType, { label: string; emoji: string; color: string }> = {
  barbell: { label: 'バーベル', emoji: '🏋️', color: '#ff6b6b' },
  dumbbell: { label: 'ダンベル', emoji: '💪', color: '#ffa94d' },
...
interface ExerciseDef {
  ...
  equipment: EquipmentType; // 使用する器具（'none'=器具なし）
  description?: string;
}
...
const ACHIEVEMENTS: Achievement[] = [
  ...
  // 大会の称号。check は常に false（大会クリア/優勝時に直接付与するため、記録セット時の自動判定では解除されない）。
  { id: 'clear_chiku', name: '地区大会入賞', description: '地区ボディビル大会で入賞する', check: () => false },
  { id: 'clear_ken', name: '県大会入賞', description: '都道府県ボディビル大会で入賞する', check: () => false },
  { id: 'clear_zenkoku', name: '全国大会入賞', description: '全国ボディビル大会で入賞する', check: () => false },
  { id: 'clear_sekai', name: '世界選手権入賞', description: '世界ボディビル選手権で入賞する', check: () => false },
  { id: 'win_sekai', name: 'グランプリ王者', description: '世界ボディビル選手権で優勝する', check: () => false },
...
const EXERCISES: ExerciseDef[] = [
  // 胸 (Chest)
  { id: 'bench_press', name: 'ベンチプレス', primaryMuscle: 'chest', equipment: 'barbell', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.5}, {muscle: 'shoulder', expRatio: 0.4}], description: '仰向けになりバーベルを胸まで下ろして押し上げる種目。大胸筋全体を強力に鍛えます。' },
  { id: 'push_up', name: '腕立て伏せ', primaryMuscle: 'chest', isBodyweight: true, equipment: 'none', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.5}, {muscle: 'abs', expRatio: 0.2}], description: '手を肩幅よりやや広くつき、体を一直線に保ったまま腕の曲げ伸ばしを行います。' },
  { id: 'dumbbell_fly', name: 'ダンベルフライ', primaryMuscle: 'chest', equipment: 'dumbbell', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'shoulder', expRatio: 0.2}], description: '仰向けでダンベルを持ち、鳥が羽ばたくように腕を開閉させ大胸筋をストレッチさせます。' },
  { id: 'chest_press', name: 'チェストプレス', primaryMuscle: 'chest', equipment: 'machine', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.4}], description: 'マシンに座り、グリップを前に押し出して大胸筋を鍛える安全な種目です。' },
  { id: 'wide_push_up', name: 'ワイドプッシュアップ', primaryMuscle: 'chest', isBodyweight: true, equipment: 'none', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'shoulder', expRatio: 0.3}, {muscle: 'triceps', expRatio: 0.3}], description: '手幅を肩幅より大きく広げて行う腕立て伏せ。大胸筋の外側を強くストレッチさせて鍛えます。' },
  { id: 'decline_push_up', name: 'デクラインプッシュアップ', primaryMuscle: 'chest', isBodyweight: true, equipment: 'none', targets: [{muscle: 'chest', expRatio: 1.0}, {muscle: 'shoulder', expRatio: 0.4}, {muscle: 'triceps', expRatio: 0.4}], description: '足を椅子やベンチに乗せ、頭を低くして行う腕立て伏せ。大胸筋上部と肩に効きます。' },

  // 背中 (Back)
  { id: 'pull_up', name: '懸垂（チンニング）', primaryMuscle: 'back', isBodyweight: true, equipment: 'bar', targets: [{muscle: 'back', expRatio: 1.0}, {muscle: 'biceps', expRatio: 0.6}, {muscle: 'rhomboids', expRatio: 0.4}], description: 'バーにぶら下がり、肩甲骨を寄せるようにして体を持ち上げます。' },
  { id: 'deadlift', name: 'デッドリフト', primaryMuscle: 'back', equipment: 'barbell', targets: [{muscle: 'erector_spinae', expRatio: 1.0}, {muscle: 'back', expRatio: 0.8}, {muscle: 'glutes', expRatio: 0.6}, {muscle: 'hamstrings', expRatio: 0.5}], description: '床にあるバーベルを、背筋を伸ばしたまま立ち上がりながら持ち上げる全身運動です。' },
  { id: 'lat_pulldown', name: 'ラットプルダウン', primaryMuscle: 'back', equipment: 'machine', targets: [{muscle: 'back', expRatio: 1.0}, {muscle: 'biceps', expRatio: 0.4}, {muscle: 'rhomboids', expRatio: 0.3}], description: 'マシンに座り、上からバーを胸の前に引き下ろして広背筋を鍛えます。' },
  { id: 'bent_over_row', name: 'ベントオーバーロウ', primaryMuscle: 'back', equipment: 'barbell', targets: [{muscle: 'back', expRatio: 1.0}, {muscle: 'rhomboids', expRatio: 0.8}, {muscle: 'erector_spinae', expRatio: 0.5}], description: '前傾姿勢でバーベルやダンベルをお腹に向かって引き上げます。' },
  { id: 'inverted_row', name: '斜め懸垂（インバーテッドロウ）', primaryMuscle: 'back', isBodyweight: true, equipment: 'bar', targets: [{muscle: 'back', expRatio: 1.0}, {muscle: 'rhomboids', expRatio: 0.7}, {muscle: 'biceps', expRatio: 0.5}, {muscle: 'trapezius', expRatio: 0.3}], description: '低い位置のバーの下に潜り込み、体を斜めに保ったまま胸を引き寄せます。懸垂ができなくても背中を鍛えられます。' },
  { id: 'superman', name: 'スーパーマン', primaryMuscle: 'erector_spinae', isBodyweight: true, equipment: 'none', targets: [{muscle: 'erector_spinae', expRatio: 1.0}, {muscle: 'back', expRatio: 0.6}, {muscle: 'glutes', expRatio: 0.4}], description: 'うつ伏せで両手両足を同時に持ち上げ、飛ぶような姿勢をキープして背面全体を鍛えます。' },

  // 僧帽筋 (Trapezius)
  { id: 'shrug', name: 'シュラッグ', primaryMuscle: 'trapezius', equipment: 'dumbbell', targets: [{muscle: 'trapezius', expRatio: 1.0}], description: '両手に重量を持ち、肩をすくめるようにして僧帽筋を鍛えます。' },
  { id: 'upright_row', name: 'アップライトロウ', primaryMuscle: 'trapezius', equipment: 'barbell', targets: [{muscle: 'trapezius', expRatio: 1.0}, {muscle: 'shoulder', expRatio: 0.6}], description: 'バーベルやダンベルを体の前に持ち、肘を高く上げるように引き上げます。' },

  // 菱形筋 (Rhomboids)
  { id: 'seated_row', name: 'シーテッドロウ', primaryMuscle: 'rhomboids', equipment: 'machine', targets: [{muscle: 'rhomboids', expRatio: 1.0}, {muscle: 'back', expRatio: 0.6}, {muscle: 'biceps', expRatio: 0.4}], description: 'マシンに座り、ケーブルをみぞおちに向かって引き、背中の中央を鍛えます。' },
  { id: 'one_hand_row', name: 'ワンハンドロウ', primaryMuscle: 'rhomboids', equipment: 'dumbbell', targets: [{muscle: 'rhomboids', expRatio: 1.0}, {muscle: 'back', expRatio: 0.8}, {muscle: 'biceps', expRatio: 0.4}], description: 'ベンチに片手と片膝をつき、もう片方の手でダンベルを引き上げます。' },

  // 脊柱起立筋 (Erector Spinae)
  { id: 'back_extension', name: 'バックエクステンション', primaryMuscle: 'erector_spinae', isBodyweight: true, equipment: 'none', targets: [{muscle: 'erector_spinae', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.5}, {muscle: 'hamstrings', expRatio: 0.4}], description: 'うつ伏せの状態から上体を反らし、脊柱起立筋を鍛えます。' },
  { id: 'good_morning', name: 'グッドモーニング', primaryMuscle: 'erector_spinae', equipment: 'barbell', targets: [{muscle: 'erector_spinae', expRatio: 1.0}, {muscle: 'hamstrings', expRatio: 0.8}, {muscle: 'glutes', expRatio: 0.6}], description: 'バーベルを肩に担ぎ、背筋を伸ばしたままお辞儀をするように上体を倒します。' },

  // 肩 (Shoulder)
  { id: 'back_press', name: 'バックプレス', primaryMuscle: 'shoulder', equipment: 'barbell', targets: [{muscle: 'shoulder', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.4}, {muscle: 'trapezius', expRatio: 0.3}], description: '首の後ろでバーベルを上下させ、三角筋を鍛えます（肩の柔軟性が必要です）。' },
  { id: 'shoulder_press', name: 'ショルダープレス', primaryMuscle: 'shoulder', equipment: 'dumbbell', targets: [{muscle: 'shoulder', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.5}, {muscle: 'chest', expRatio: 0.2}], description: '鎖骨の前から頭上へダンベルやバーベルを押し上げます。' },
  { id: 'side_raise', name: 'サイドレイズ', primaryMuscle: 'shoulder', equipment: 'dumbbell', targets: [{muscle: 'shoulder', expRatio: 1.0}], description: '両手にダンベルを持ち、腕を横に広げて持ち上げ、肩の横側を鍛えます。' },
  { id: 'front_raise', name: 'フロントレイズ', primaryMuscle: 'shoulder', equipment: 'dumbbell', targets: [{muscle: 'shoulder', expRatio: 1.0}, {muscle: 'chest', expRatio: 0.2}], description: '両手にダンベルを持ち、腕を前に向かって持ち上げ、肩の前側を鍛えます。' },
  { id: 'pike_push_up', name: 'パイクプッシュアップ', primaryMuscle: 'shoulder', isBodyweight: true, equipment: 'none', targets: [{muscle: 'shoulder', expRatio: 1.0}, {muscle: 'triceps', expRatio: 0.5}, {muscle: 'chest', expRatio: 0.2}], description: 'お尻を高く突き上げた「く」の字姿勢で頭を床に近づけ、自重で三角筋を鍛えます。' },
  ...
  // 上腕三頭筋 (Triceps)
  { id: 'french_press', name: 'フレンチプレス', primaryMuscle: 'triceps', equipment: 'dumbbell', targets: [{muscle: 'triceps', expRatio: 1.0}], description: '頭上で重量を持ち、肘を曲げて頭の後ろに下ろし、上腕三頭筋を鍛えます。' },
  { id: 'kick_back', name: 'キックバック', primaryMuscle: 'triceps', equipment: 'dumbbell', targets: [{muscle: 'triceps', expRatio: 1.0}], description: '前傾姿勢で肘を固定し、腕を後ろに伸ばすようにダンベルを動かします。' },
  { id: 'dips', name: 'ディップス', primaryMuscle: 'triceps', isBodyweight: true, equipment: 'bar', targets: [{muscle: 'triceps', expRatio: 1.0}, {muscle: 'chest', expRatio: 0.6}, {muscle: 'shoulder', expRatio: 0.3}], description: '平行なバーに両手をつき、体を沈めてから押し上げる種目です。' },
  { id: 'narrow_bench_press', name: 'ナローベンチプレス', primaryMuscle: 'triceps', equipment: 'barbell', targets: [{muscle: 'triceps', expRatio: 1.0}, {muscle: 'chest', expRatio: 0.5}, {muscle: 'shoulder', expRatio: 0.3}], description: '手幅を狭く握って行うベンチプレス。上腕三頭筋に強力な刺激を与えます。' },
  { id: 'bench_dips', name: 'ベンチディップス', primaryMuscle: 'triceps', isBodyweight: true, equipment: 'none', targets: [{muscle: 'triceps', expRatio: 1.0}, {muscle: 'shoulder', expRatio: 0.3}, {muscle: 'chest', expRatio: 0.2}], description: '椅子やベンチに手をつき、体を沈めてから押し上げます。ディップスより手軽な自重の三頭筋種目です。' },
  ...
  { id: 'wrist_curl', name: 'リストカール', primaryMuscle: 'forearm_flexors', equipment: 'dumbbell', targets: [{muscle: 'forearm_flexors', expRatio: 1.0}], description: '前腕をベンチに固定し、手首を上に曲げて前腕の内側（屈筋群）を鍛えます。' },
  { id: 'reverse_wrist_curl', name: 'リバースリストカール', primaryMuscle: 'forearm_flexors', equipment: 'dumbbell', targets: [{muscle: 'forearm_flexors', expRatio: 1.0}], description: '手のひらを下に向けて手首を持ち上げ、前腕外側・手首を伸ばす筋肉を鍛えます。' },
  { id: 'dead_hang', name: 'デッドハング（ぶら下がり）', primaryMuscle: 'forearm_flexors', isBodyweight: true, equipment: 'bar', targets: [{muscle: 'forearm_flexors', expRatio: 1.0}, {muscle: 'brachioradialis', expRatio: 0.5}], description: 'バーに全体重でぶら下がり続けます。握力と前腕、腕橈骨筋を自重で鍛える種目です。' },

  // お尻 (Glutes)
  { id: 'hip_thrust', name: 'ヒップスラスト', primaryMuscle: 'glutes', equipment: 'barbell', targets: [{muscle: 'glutes', expRatio: 1.0}, {muscle: 'hamstrings', expRatio: 0.4}], description: '仰向けで肩をベンチに乗せ、バーベルを骨盤に乗せてお尻を持ち上げます。' },
  { id: 'back_kick', name: 'バックキック', primaryMuscle: 'glutes', isBodyweight: true, equipment: 'none', targets: [{muscle: 'glutes', expRatio: 1.0}, {muscle: 'hamstrings', expRatio: 0.3}], description: '四つん這いになり、片足を後ろへ蹴り上げるようにお尻を収縮させます。' },
  { id: 'bulgarian_squat', name: 'ブルガリアンスクワット', primaryMuscle: 'glutes', isBodyweight: true, equipment: 'none', targets: [{muscle: 'glutes', expRatio: 1.0}, {muscle: 'legs', expRatio: 0.8}, {muscle: 'hamstrings', expRatio: 0.5}], description: '片足を後ろのベンチに乗せ、もう片方の足で深くしゃがみ込みます。' },
  { id: 'glute_bridge', name: 'ヒップリフト（グルートブリッジ）', primaryMuscle: 'glutes', isBodyweight: true, equipment: 'none', targets: [{muscle: 'glutes', expRatio: 1.0}, {muscle: 'hamstrings', expRatio: 0.4}], description: '仰向けで膝を立て、お尻を天井に向けて持ち上げます。器具なしで大殿筋を鍛えられる定番種目です。' },

  // 中殿筋 (Gluteus Medius)
  { id: 'abduction', name: 'アブダクション', primaryMuscle: 'gluteus_medius', equipment: 'machine', targets: [{muscle: 'gluteus_medius', expRatio: 1.0}], description: 'マシンに座り、膝を外側に開く動きでお尻の横側（中殿筋）を鍛えます。' },
  { id: 'clamshell', name: 'クラムシェル', primaryMuscle: 'gluteus_medius', isBodyweight: true, equipment: 'none', targets: [{muscle: 'gluteus_medius', expRatio: 1.0}], description: '横向きに寝て、貝殻が開くように上の膝を開き中殿筋を鍛えます。' },
  { id: 'side_leg_raise', name: 'サイドレッグレイズ', primaryMuscle: 'gluteus_medius', isBodyweight: true, equipment: 'none', targets: [{muscle: 'gluteus_medius', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.3}], description: '横向きに寝て、上側の脚をまっすぐ伸ばしたまま真上に持ち上げます。お尻の横（中殿筋）に効きます。' },

  // 脚・大腿四頭筋 (Legs)
  { id: 'squat', name: 'スクワット', primaryMuscle: 'legs', isBodyweight: true, equipment: 'none', targets: [{muscle: 'legs', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.8}, {muscle: 'hamstrings', expRatio: 0.5}, {muscle: 'erector_spinae', expRatio: 0.3}], description: '足を肩幅に開き、背筋を伸ばしたまま深くしゃがみ込む下半身の王様です。' },
  { id: 'leg_press', name: 'レッグプレス', primaryMuscle: 'legs', equipment: 'machine', targets: [{muscle: 'legs', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.6}], description: 'マシンに座り、足でプレートを押し上げて脚全体を鍛えます。' },
  { id: 'leg_extension', name: 'レッグエクステンション', primaryMuscle: 'legs', equipment: 'machine', targets: [{muscle: 'legs', expRatio: 1.0}], description: 'マシンに座り、膝を伸ばす動きで太ももの前側（大腿四頭筋）を鍛えます。' },
  { id: 'lunge', name: 'ランジ', primaryMuscle: 'legs', isBodyweight: true, equipment: 'none', targets: [{muscle: 'legs', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.9}, {muscle: 'hamstrings', expRatio: 0.6}], description: '足を前後に開き、後ろの膝が床につく直前まで沈み込んでから立ち上がります。' },
  { id: 'jump_squat', name: 'ジャンプスクワット', primaryMuscle: 'legs', isBodyweight: true, equipment: 'none', targets: [{muscle: 'legs', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.7}, {muscle: 'hamstrings', expRatio: 0.4}], description: 'スクワットで沈み込んだ勢いでジャンプし、着地したらすぐ次へ。爆発力と脚の筋力を自重で鍛えます。' },
  { id: 'wall_sit', name: 'ウォールシット（空気椅子）', primaryMuscle: 'legs', isBodyweight: true, equipment: 'none', targets: [{muscle: 'legs', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.3}], description: '壁に背中をつけ、太ももが床と平行になる姿勢をキープします。器具なしで大腿四頭筋を追い込めます。' },

  // 股関節内転筋群 (Adductors)
  { id: 'wide_squat', name: 'ワイドスクワット', primaryMuscle: 'adductors', isBodyweight: true, equipment: 'none', targets: [{muscle: 'adductors', expRatio: 1.0}, {muscle: 'legs', expRatio: 0.8}, {muscle: 'glutes', expRatio: 0.6}], description: '足幅を広めに開き、つま先を外側に向けて行うスクワット。内もも（内転筋）を強力に刺激します。' },
  { id: 'adduction', name: 'アダクション', primaryMuscle: 'adductors', equipment: 'machine', targets: [{muscle: 'adductors', expRatio: 1.0}], description: '専用マシンに座り、両脚を外側から内側に閉じる動きで内転筋群を集中強化します。' },
  { id: 'side_lunge', name: 'サイドランジ', primaryMuscle: 'adductors', isBodyweight: true, equipment: 'none', targets: [{muscle: 'adductors', expRatio: 1.0}, {muscle: 'legs', expRatio: 0.7}, {muscle: 'glutes', expRatio: 0.5}], description: '横方向に大きく一歩を踏み出し、股関節を折りたたんで内ももと太ももを鍛えます。' },
  { id: 'copenhagen_plank', name: 'コペンハーゲンプランク', primaryMuscle: 'adductors', isBodyweight: true, equipment: 'none', targets: [{muscle: 'adductors', expRatio: 1.0}, {muscle: 'obliques', expRatio: 0.5}], description: 'ベンチに片足を乗せて横向きで体を支える自重種目。内もものインナーマッスルを鍛えます。' },

  // ハムストリングス (Hamstrings)
  { id: 'leg_curl', name: 'レッグカール', primaryMuscle: 'hamstrings', equipment: 'machine', targets: [{muscle: 'hamstrings', expRatio: 1.0}], description: 'マシンにうつ伏せになり、膝を曲げてかかとをお尻に近づけハムストリングスを鍛えます。' },
  { id: 'romanian_deadlift', name: 'ルーマニアンデッドリフト', primaryMuscle: 'hamstrings', equipment: 'barbell', targets: [{muscle: 'hamstrings', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.8}, {muscle: 'erector_spinae', expRatio: 0.6}], description: '膝を軽く曲げたまま、お尻を後ろに引きながら上体を倒し裏ももを伸ばします。' },
  { id: 'nordic_curl', name: 'ノルディックハムストリングカール', primaryMuscle: 'hamstrings', isBodyweight: true, equipment: 'none', targets: [{muscle: 'hamstrings', expRatio: 1.0}, {muscle: 'glutes', expRatio: 0.4}], description: '足首を固定して膝立ちになり、体をまっすぐ保ったままゆっくり前に倒れます。自重最強クラスの裏もも種目です。' },

  // 腹直筋 (Abs)
  { id: 'crunch', name: 'クランチ', primaryMuscle: 'abs', isBodyweight: true, equipment: 'none', targets: [{muscle: 'abs', expRatio: 1.0}], description: '仰向けになり、腰を床につけたままおへそを覗き込むように上体を丸めます。' },
  { id: 'ab_roller', name: '腹筋ローラー (アブローラー)', primaryMuscle: 'abs', isBodyweight: true, equipment: 'roller', targets: [{muscle: 'abs', expRatio: 1.0}, {muscle: 'transversus_abdominis', expRatio: 0.8}, {muscle: 'back', expRatio: 0.4}], description: '膝をついてローラーを持ち、体を前に伸ばしてから元の位置に戻ります。' },

  // 腹斜筋 (Obliques)
  { id: 'side_crunch', name: 'サイドクランチ', primaryMuscle: 'obliques', isBodyweight: true, equipment: 'none', targets: [{muscle: 'obliques', expRatio: 1.0}, {muscle: 'abs', expRatio: 0.4}], description: '横向きに寝て、脇腹を縮めるように上体を起こします。' },
  { id: 'russian_twist', name: 'ロシアンツイスト', primaryMuscle: 'obliques', isBodyweight: true, equipment: 'none', targets: [{muscle: 'obliques', expRatio: 1.0}, {muscle: 'abs', expRatio: 0.5}], description: '体育座りの姿勢で少し上体を倒し、胴体を左右にひねります。' },

  // 腸腰筋 (Iliopsoas)
  { id: 'bicycle_crunch', name: 'バイシクルクランチ', primaryMuscle: 'iliopsoas', isBodyweight: true, equipment: 'none', targets: [{muscle: 'obliques', expRatio: 1.0}, {muscle: 'iliopsoas', expRatio: 0.8}, {muscle: 'abs', expRatio: 0.6}], description: '仰向けで自転車を漕ぐように足を動かし、対角の肘と膝を近づけます。' },
  { id: 'leg_raise', name: 'レッグレイズ', primaryMuscle: 'iliopsoas', isBodyweight: true, equipment: 'none', targets: [{muscle: 'iliopsoas', expRatio: 1.0}, {muscle: 'abs', expRatio: 0.8}], description: '仰向けで足を揃え、床から垂直になるまで下腹部の力で持ち上げます。' },

  // 腹横筋 (Transversus Abdominis)
  { id: 'draw_in', name: 'ドローイン (自重設定)', primaryMuscle: 'transversus_abdominis', isBodyweight: true, equipment: 'none', targets: [{muscle: 'transversus_abdominis', expRatio: 1.0}], description: 'お腹を極限までへこませ、その状態をキープしてインナーマッスルを鍛えます。' },
  { id: 'plank', name: 'プランク (自重設定)', primaryMuscle: 'transversus_abdominis', isBodyweight: true, equipment: 'none', targets: [{muscle: 'transversus_abdominis', expRatio: 1.0}, {muscle: 'abs', expRatio: 0.5}, {muscle: 'shoulder', expRatio: 0.2}, {muscle: 'triceps', expRatio: 0.2}], description: '肘とつま先で体を支え、体が一直線になる姿勢をキープします。' },
];
...
const MUSCLE_NICKNAME_SAMPLES: Record<MuscleType, string> = {
  ...
  back: 'アメリア',              // 女性初の大西洋単独横断飛行士アメリア・イアハートより。背に広がる巨大な翼の象徴
  shoulder: 'パトリック',        // 「高貴な守護者」の意。肩を覆う強固な防具のイメージ
  biceps: 'ヘラクレス',          // ギリシャ神話の英雄。ヘラクレスオオカブトと最強の力こぶに
  triceps: 'フィリッパ',         // フィリップの女性形で意味は同じ「馬を愛する者」。馬蹄形とペガサスに由来
  brachioradialis: 'ボビー',     // 大工ビーバーのイメージ。ハンマーを握って木を加工する
  forearm_flexors: 'ブリジット', // 力と技を司る女神ブリギッドに由来。物を握り潰すほどの前腕
  glutes: 'アトラス',            // 地球を支える巨人。人体最大のパワーを生むお尻に
  legs: 'ジャック',              // 豪州でカンガルーのオスの愛称。跳躍力抜群のキックボクサー
  hamstrings: 'フローレンス',    // 女子100mの世界記録保持者フローレンス・ジョイナーより。爆走とブレーキを制御するもも裏
  ...
  obliques: 'マリウス',          // マリアの男性形。ウエストが締まった凄腕の刺客。スズメバチのシャープなくびれ
  iliopsoas: 'リサ',             // WNBA初のダンクを決めたリサ・レスリーより。もも上げのバネの象徴
  transversus_abdominis: 'シェリダン', // 内側から全身を締め支える寡黙な兵。天然のコルセット
  trapezius: 'ベネディクタ',     // 修道院創設者ベネディクトの女性形。語源の「修道士のフード」に由来
  erector_spinae: 'リュウ',      // 東洋の「龍」。背骨をまっすぐ支える龍のイメージ
  rhomboids: 'アンジェロ',       // 「天使」の意（アンジェラの男性形）。肩甲骨の間の「天使の羽」を動かすヒンジ
};
...
async function saveNotifySnapshot(stats: AppState, playerName: string, enabled = true): Promise<void> {
  ...
  const muscles = (Object.keys(stats) as MuscleType[])
    ...
    .map(m => {
      ...
      const nick = stats[m].nickname || MUSCLE_NAMES[m];
      // 通知本文に使うキャラのセリフを、{name}/{nick} を埋めて事前生成しておく
      // （SW は CHARACTER_LINES を参照できないため、テンプレート済みの文字列を渡す）。
      const tmpl = (s: string) => s.replace(/\{name\}/g, playerName).replace(/\{nick\}/g, nick);
      const voice = CHARACTER_LINES[m];
      return {
        id: m,
        name: nick,
        ...
        // 鍛えどき（回復完了）通知用のセリフ候補と、狙い目もうすぐ終了通知用のセリフ候補。
        readyLines: voice.superComp.map(tmpl),
        endingLines: (voice.peakEnding ?? voice.superComp).map(tmpl),
      };
    });
  try {
...
const CHARACTER_LINES: Record<MuscleType, CharacterVoice> = {
  ...
  back: {
    condition: [
      { min: 85, lines: ['絶好調よ、{name}。今なら大きな翼を広げて、どこまでも飛べそう。', 'みなぎってるわ…！{name}、この背中で君を空へ連れて行ってあげる。', '絶好調よ、{name}。翼が軽いの。今日はどこまでも飛べる気がするわ。'] },
      { min: 65, lines: ['いい風が吹いてるわ、{name}。翼がぐんぐん育ってるの。', '背中に上昇気流を感じるの、{name}。いい調子よ。'] },
      { min: 40, lines: ['まあまあの空模様ね。{name}、そろそろ羽ばたきの練習をしましょうか。', 'まずまずの空模様よ。{name}、そろそろ懸垂で羽ばたきましょう。'] },
      { min: 20, lines: ['少し翼が重いの…{name}、今日はゆっくり休ませてほしいわ。', '向かい風が強いわね…{name}、今日は低く飛んでおくわ。'] },
      { min: 0, lines: ['翼が上がらないの…{name}、回復するまで空はお預けね…', '翼を広げる力もないわ…{name}、地上でゆっくり休ませて。'] },
    ],
    phase: {
      1: ['わたしの翼はまだ小さいけど、いつか大空を制すのが夢なの。頼むわね{name}。', '今はまだ雛だけど、いつか大空を舞うわ。見ていてね、{name}。'],
      2: ['背中が広がってきたでしょう、{name}？翼の面積が増えてきたわ。', '翼が風をつかむ感覚がわかってきたの、{name}。もっと広がるわ。'],
      3: ['見てごらんなさい{name}、この大きな翼を。逆三角形の空へ羽ばたきましょう。', 'この翼があれば、逆三角形の大空も自由自在よ、{name}。'],
    },
  ...
  triceps: {
    condition: [
      { min: 85, lines: ['調子はいいわよ…って言っても、どうせ二頭筋ばっか褒められるんでしょ、{name}。', '絶好調だけどね。腕の太さの2/3はあたしなんだから、{name}、忘れないでよ。', '絶好調よ、{name}。…って、また二頭筋の話？あたしの話をしましょ。'] },
      { min: 65, lines: ['まあ悪くないわ。{name}、たまには裏側のあたしも見てよね。', '悪くない調子よ。{name}、腕の裏側、地味に頼れるでしょ？'] },
      { min: 40, lines: ['ぼちぼちよ。{name}、二の腕たぷたぷが嫌ならあたしを鍛えることね。', 'まあ普通ね。{name}、キックバックであたしを思い出してよ。'] },
      { min: 20, lines: ['ちょっと疲れたわ…{name}、地味に頑張ってるあたしを休ませて。', '疲れたわ…{name}、目立たない分、気づかれず酷使されがちなのよ。'] },
      { min: 0, lines: ['もう限界…どうせ誰も気づかないでしょうけど、あたし、限界なの{name}…', '限界よ…{name}、縁の下も、たまには休まないと壊れちゃうんだから。'] },
    ],
    phase: {
      1: ['あたしはまだ細いけどね、腕を太くしたいならあたしが本命よ、{name}。', 'まだ細いけどね、腕の太さはあたし次第。期待してていいわよ、{name}。'],
      2: ['二の腕に馬蹄が浮いてきたでしょ？{name}、地味だけど確実に育ってるわ。', '二の腕の裏に馬蹄が浮いてきた。{name}、あたしの実力、わかった？'],
      3: ['見た{name}？この馬蹄形。結局、腕を太くしたのはあたしなのよ。', 'この馬蹄形、腕の主役の証よ。{name}、もう二頭筋には負けないわ。'],
    },
  ...
  brachioradialis: {
    condition: [
      { min: 85, lines: ['…絶好調だよ。{name}、次の仕事、任せな。', '手が鳴るね…{name}、いい木を持ってきな。加工してやる。', '…体が動く。{name}、今日はいい仕事ができそうだ。'] },
      { min: 65, lines: ['悪くないよ。{name}、ハンマーなら、まだ握れるわ。', '悪くない。{name}、リバースカール、始めようか。'] },
      { min: 40, lines: ['まあまあだね。{name}、そろそろ一仕事、いくよ。', '…まあ、こんなもんさ。{name}、ぼちぼちやろうか。'] },
      { min: 20, lines: ['…疲れた。{name}、少し道具を置かせて。', '…手が重い。{name}、今日は無理せずいこう。'] },
      { min: 0, lines: ['手に力が入らないよ…{name}、今日は仕舞いだ。', '…もう握れない。{name}、道具を置くよ。すまないね。'] },
    ],
    phase: {
      1: ['あたしはまだ半人前。{name}、一人前の職人にしておくれ。', '…まだ駆け出しさ。{name}、腕のいい職人にしておくれ。'],
      2: ['腕に厚みが出てきた。{name}、いい仕事ができそうだ。', '前腕に厚みが出てきた。{name}、腕がいい道具になってきたね。'],
      3: ['見な、この前腕。{name}、どんな硬い木でも通してやる。', '…この前腕、一人前だ。{name}、どんな仕事も任せな。'],
    },
  ...
  forearm_flexors: {
    condition: [
      { min: 85, lines: ['絶好調だ！{name}、今のあたしなら何を握っても離さないわよ！', 'みなぎってるわ！{name}、握力なら誰にも負けない！', '絶好調よ{name}！今なら岩だって握り潰せる気がするわ！'] },
      { min: 65, lines: ['いい握り心地だわ。{name}、バーベル、ぶら下げてきなさい！', '握りに力がこもるわ、{name}。デッドリフト、いってみる？'] },
      { min: 40, lines: ['まあまあね。{name}、そろそろ握り込みたくなってきたわ。', 'まあまあよ、{name}。リストカールで握力を仕込みましょ。'] },
      { min: 20, lines: ['握力が落ちてきた…{name}、ちょっと休ませて。', '握力が心もとないわ…{name}、今日は握り込みは控えとくわ。'] },
      { min: 0, lines: ['もう握れない…{name}、指一本動かせないの…', 'もう指が閉じないわ…{name}、握力の限界よ、休ませて。'] },
    ],
    phase: {
      1: ['あたしの握力はまだまだ。{name}、一度掴んだら離さない握りにしてちょうだい！', 'あたしの握りはまだ甘いわ。{name}、万力みたいな前腕にしてちょうだい！'],
      2: ['前腕が太くなってきたでしょ？{name}、握る力は裏切らないわ。', '前腕に血管が浮いてきたでしょ？{name}、握りが本物になってきたわ。'],
      3: ['見なさい{name}、この前腕。あたしが掴んだら、もう二度と離れない。', 'この握力、もう誰にも解けないわ。{name}、頼れる相棒でしょ？'],
    },
  ...
  hamstrings: {
    condition: [
      { min: 85, lines: ['絶好調だよ{name}！でも飛ばす前に、ちゃんと準備運動しようね。', 'いい感じに温まってるわ。{name}、今なら安全に爆走できるよ！', '絶好調だよ{name}！でも爆発力を出す前に、入念にストレッチね。'] },
      { min: 65, lines: ['調子は悪くないよ。{name}、でも急な高重量は禁物だからね。', 'いい張り具合だよ、{name}。焦らずルーマニアンデッドリフトを。'] },
      { min: 40, lines: ['まあまあかな。{name}、慌てず、じっくりレッグカールでいこう。', 'まあまあかな。{name}、もも裏はゆっくり伸ばすのが大事なの。'] },
      { min: 20, lines: ['ちょっと張ってるの…{name}、無理すると肉離れするから休ませて。', '結構張ってるの…{name}、ここで無理したら一発で肉離れだよ。'] },
      { min: 0, lines: ['危ない、これ以上は肉離れしちゃう…{name}、絶対に休もうね。', 'これ以上は本当に危険だよ…{name}、今日は完全休養しましょう。'] },
    ],
    phase: {
      1: ['わたしはまだ頼りないけど、速くて怪我しない脚裏を目指すね、{name}。', 'まだ頼りないけど、速くしなやかな脚裏になるわ。見ててね{name}。'],
      2: ['もも裏に粘りが出てきたよ{name}。ブレーキ性能が上がってきた。', 'もも裏に弾力が出てきたよ{name}。ブレーキもよく効くようになったの。'],
      3: ['見て{name}、このもも裏。速さもブレーキも、わたしに任せて。', 'このもも裏、加速もブレーキも完璧よ。{name}、安心して任せて。'],
    },
  ...
  gluteus_medius: {
    condition: [
      { min: 85, lines: ['絶好調ね、{name}。今なら骨盤を完璧に水平に保てるわ。', 'コンディションは最良よ。{name}、バランスはわたしに任せて。', '絶好調よ、{name}。今のわたしなら、片足でも一切ブレないわ。'] },
      { min: 65, lines: ['状態は良好ね。{name}、片足立ちも安定しているわ。', '安定感は良好ね、{name}。横方向の動きも軽やかよ。'] },
      { min: 40, lines: ['まずまずといったところ。{name}、そろそろ横の動きで鍛えましょうか。', '平均的な状態ね。{name}、クラムシェルで横の意識を入れましょう。'] },
      { min: 20, lines: ['少しバランスが崩れてきた…{name}、休息を入れた方がいいわ。', 'バランスが乱れ始めた…{name}、ここで休むのが賢明な判断よ。'] },
      { min: 0, lines: ['もう体を支えきれない…{name}、今は無理をさせないで。', '軸を保てない…{name}、無理は転倒のもと。休ませて。'] },
    ],
    phase: {
      1: ['わたしはまだ弱いバランサー。{name}、安定した骨盤を作ってほしいの。', 'まだ制御が甘いバランサーよ。{name}、安定した骨盤を作りましょう。'],
      2: ['横の安定感が増してきたわね、{name}。歩きのブレが減っているわ。', '歩行時の揺れが減ってきたわね、{name}。制御精度が上がっているの。'],
      3: ['見てほしいの、この安定感。{name}、体の軸はわたしが制御している。', 'この安定性、まさに人体のジャイロよ。{name}、軸はわたしが守る。'],
    },
  ...
  obliques: {
    condition: [
      { min: 85, lines: ['絶好調だ、{name}。今の俺のくびれ、隙がない。', 'キレてるぜ。{name}、ひとひねりで仕留めてみせる。', '絶好調だ、{name}。今の俺のくびれ、寸分の隙もない。'] },
      { min: 65, lines: ['悪くない。{name}、脇腹のラインが冴えてきた。', 'キレは十分だ、{name}。ロシアンツイストで刻んでいくぞ。'] },
      { min: 40, lines: ['まあまあだな。{name}、そろそろツイストで刻むか。', 'まあ及第点だ。{name}、脇腹、そろそろ締め直したい。'] },
      { min: 20, lines: ['少し疲れた…{name}、今は休ませてくれ。', '少し鈍ってる…{name}、今日は深追いしないでおく。'] },
      { min: 0, lines: ['もう動けない…{name}、深追いは無用だ。', '刃こぼれだ…{name}、無理な追い込みは命取りになる。'] },
    ],
    phase: {
      1: ['俺はまだ甘い。{name}、鋭いくびれに仕込んでくれ。', 'まだ切れ味が甘い。{name}、鋭いくびれに研ぎ澄ましてくれ。'],
      2: ['脇腹に斜めのラインが出てきたな、{name}。キレが増した。', '脇腹に斜めのラインが浮いてきた。{name}、刃が冴えてきたな。'],
      3: ['見ろ{name}、このVシェイプ。無駄のない刃だ。', 'このVシェイプ、一分の狂いもない刃だ。{name}、惚れたか？'],
    },
  ...
  iliopsoas: {
    condition: [
      { min: 85, lines: ['…絶好調よ。{name}、いざとなれば、この跳躍を見せてあげる。', '力は満ちている。{name}、あたしのバネ、甘く見ないで。', '…調子はいいわ。{name}、今なら膝が胸まで跳ね上がる。'] },
      { min: 65, lines: ['悪くない。{name}、上げようと思えば、膝はどこまでも上がるわ。', '悪くないわ。{name}、脚を高く引き上げてみる？'] },
      { min: 40, lines: ['まあまあね。{name}、そろそろ脚を高く上げてみましょうか。', '…普通よ。{name}、レッグレイズで、じっくり目覚めさせて。'] },
      { min: 20, lines: ['…少し重いわ。{name}、今は跳ばずにおく。', '…バネが鈍い。{name}、今日は無理に跳ばない。'] },
      { min: 0, lines: ['脚が上がらない…{name}、今日は静かにさせて。', '脚が持ち上がらない…{name}、今は静かに力を蓄える時よ。'] },
    ],
    phase: {
      1: ['あたしはまだ眠れるバネ。{name}、驚異の跳躍を仕込んでちょうだい。', '…あたしは眠れる力。{name}、驚異のバネを目覚めさせて。'],
      2: ['膝が高く上がるようになったでしょう、{name}。バネが目覚めてきた。', '膝が高く上がるようになった。{name}、バネが鳴り始めたわ。'],
      3: ['…見た{name}？この跳躍。上と下を繋ぐのは、あたしだけよ。', '…この跳躍、体を繋ぐあたしだけの技。{name}、見逃さないで。'],
    },
  ...
  transversus_abdominis: {
    condition: [
      { min: 85, lines: ['絶好調です、{name}さん。今日はしっかり内側から支えますよ。', '調子がいいんです。{name}さん、お腹、きゅっと締めておきます。', '絶好調です、{name}さん。今日はお腹の奥まで、しっかり効いてます。'] },
      ...
      { min: 20, lines: ['少し疲れました…{name}さん、そっと休ませてください。', 'ちょっと疲れました…{name}さん、深呼吸して、ゆっくりいきましょう。'] },
      { min: 0, lines: ['すみません、もう支えきれなくて…{name}さん、休ませてください…', 'もう力が入らなくて…{name}さん、すみません、休ませてください。'] },
    ],
    phase: {
      1: ['俺はまだ薄いコルセット。{name}さん、内臓を支える力をください。', 'まだ薄いんです。{name}さん、しっかりしたコルセットに育ててください。'],
      2: ['お腹の奥が締まってきましたね、{name}さん。ぽっこりが減りましたよ。', 'お腹まわりが引き締まってきましたね、{name}さん。少し嬉しいです。'],
      3: ['見えないところですが、{name}さん、しっかり内側を支えています。', '見えないけど、{name}さんの内臓、俺がしっかり守っています。'],
    },
  ...
  trapezius: {
    condition: [
      { min: 85, lines: ['絶好調ですわ、{name}殿。肩こり知らずの好調ぶりですのよ。', '力がみなぎっておりますわ。{name}殿、この肩、頼りになさいまし。', '絶好調ですわ、{name}殿。首も肩も、羽のように軽うございます。'] },
      { min: 65, lines: ['調子は良好ですわ。{name}殿、首も肩も軽うございます。', '調子は上々ですわ、{name}殿。シュラッグ、いってみましょうか。'] },
      { min: 40, lines: ['まずまずですわね。{name}殿、そろそろシュラッグで動かしましょうか。', 'まずまずですわ。{name}殿、たまには肩を回してやってくださいまし。'] },
      { min: 20, lines: ['やれやれ、少々こってきましたわね…{name}殿、休息を。', 'やれやれ、また凝ってきましたわ…{name}殿、少し休ませてくださいまし。'] },
      { min: 0, lines: ['ああ、肩が重い…{name}殿、この老骨、休ませてくださいまし…', 'ああ、肩が石のよう…{name}殿、この老骨、もう限界ですわ。'] },
    ],
    phase: {
      1: ['わたくしはまだ頼りない肩。{name}殿、こりに負けぬ僧帽筋に育ててくださいまし。', 'まだ心もとない肩ですわ。{name}殿、凝りに負けぬよう鍛えてくださいまし。'],
      2: ['首から肩に厚みが出てまいりましたわね、{name}殿。血の巡りも良好ですわ。', '首から肩に山ができてまいりましたわ、{name}殿。頼もしい限り。'],
      3: ['この肩、もはや盤石ですわ、{name}殿。こりなど寄せ付けませぬ。', 'この肩、もはや鉄壁ですわ、{name}殿。凝りとは無縁の境地ですのよ。'],
    },
  ...
  rhomboids: {
    condition: [
      { min: 85, lines: ['絶好調だぜ{name}！今日は羽をぴんと開いて、いい姿勢でいこうぜ！', 'すごく調子がいいんだ。{name}、肩甲骨、きゅっと寄せてやるぜ！', '絶好調だ{name}！今日は羽を大きく開いて、胸を張っていこうぜ！'] },
      { min: 65, lines: ['いい感じだぜ、{name}。天使の羽、しっかり動いてるぜ。', '調子いいぜ、{name}。肩甲骨がなめらかに動いてるんだ。'] },
      { min: 40, lines: ['まずまずだな。{name}、そろそろロウイングで羽を動かそうぜ。', 'まずまずだな。{name}、シーテッドロウで羽を寄せてやるぜ。'] },
      { min: 20, lines: ['少し疲れちまった…{name}、ちょっとだけ休ませてくれ。', 'ちょっと疲れちまった…{name}、少しだけ羽を休ませてくれ。'] },
      { min: 0, lines: ['すまねぇ、もう羽が動かないんだ…{name}、休ませてくれ…', 'すまねぇ、羽が上がらないんだ…{name}、今日は休ませてくれ。'] },
    ],
    phase: {
      1: ['俺はまだ小さな羽。{name}、猫背を直せる菱形筋に育ててくれ！', 'まだ小さな羽だ。{name}、猫背を直せる立派な羽に育ててくれ！'],
      2: ['肩甲骨がしっかり寄るようになってきたな、{name}。姿勢が綺麗だぜ。', '肩甲骨がしっかり寄るようになったな、{name}。姿勢が見違えたぜ。'],
      3: ['見ろ{name}、この天使の羽。お前の背中、いつでもぴんと伸ばしてやる。', 'この天使の羽で、{name}の背中はいつでもぴんっと真っ直ぐだ。'],
    },
...
// 種目が使う器具を示すバッジ。器具なし（自重）か、器具の種類が一目で分かる。
function EquipmentBadge({ equipment, size = 'md' }: { equipment: EquipmentType; size?: 'sm' | 'md' }) {
  ...
  return (
    <span
      style={{
        ...
        fontSize: small ? '0.65rem' : '0.75rem', fontWeight: 'bold',
        color: info.color,
...
// ============================================================================
// ===== ボディビル大会（コンテスト）システム =====
// 育てた全身のモンスター＝「あなたの肉体」で規定ポーズを決め、審査ポイントを稼ぐ。
// 実際の審査3軸に対応：筋肉量＝レベル / 仕上がり＝調子・鍛えどき / バランス＝部位グループの均整。
// 大会ではEXPを一切配らない（ストリークが称号のみなのと同じ思想）。報酬は入賞・優勝の称号。
// ============================================================================

// 部位が「回復中（オーバーワーク圏）」か（大会採点用のモジュール版。Date依存だが純粋）。
...
// 部位が「超回復ピーク（鍛えどき）」か（大会採点用のモジュール版）。
...
// 規定ポーズ。displays＝そのポーズで審査員に見せる（得点対象の）部位と重み（主役ほど大きい）。
interface PoseDef {
  id: string;
  name: string;
  emoji: string;
  hint: string; // どこを見せるポーズか
  displays: { muscle: MuscleType; weight: number }[];
}

const CONTEST_POSES: PoseDef[] = [
  {
    id: 'front_double_biceps',
    name: 'フロント・ダブルバイセップス',
    emoji: '💪',
    hint: '力こぶを中心に全身のX-frameを見せる',
    displays: [
      { muscle: 'biceps', weight: 1.0 },
      { muscle: 'brachioradialis', weight: 0.5 },
      { muscle: 'forearm_flexors', weight: 0.4 },
      { muscle: 'shoulder', weight: 0.4 },
      { muscle: 'abs', weight: 0.3 },
      { muscle: 'legs', weight: 0.3 },
    ],
  },
  {
    id: 'front_lat_spread',
  ...
  },
  {
    ...
    name: 'サイドチェスト',
    emoji: '🛡️',
    hint: '大胸筋の厚みと腕・脚のセパレーションを見せる',
    displays: [
      { muscle: 'chest', weight: 1.0 },
      { muscle: 'shoulder', weight: 0.5 },
      { muscle: 'triceps', weight: 0.4 },
      { muscle: 'forearm_flexors', weight: 0.3 },
      { muscle: 'legs', weight: 0.3 },
    ],
  },
  {
    ...
    displays: [
      ...
      { muscle: 'brachioradialis', weight: 0.4 },
      { muscle: 'shoulder', weight: 0.3 },
      { muscle: 'abs', weight: 0.3 },
    ],
  },
  {
    id: 'back_double_biceps',
    name: 'バック・ダブルバイセップス',
    emoji: '🦍',
    hint: '背中全体の厚みと下半身後面を見せる',
    displays: [
      { muscle: 'back', weight: 1.0 },
      { muscle: 'trapezius', weight: 0.6 },
      { muscle: 'erector_spinae', weight: 0.5 },
      { muscle: 'rhomboids', weight: 0.5 },
      { muscle: 'biceps', weight: 0.4 },
      { muscle: 'hamstrings', weight: 0.4 },
      { muscle: 'glutes', weight: 0.3 },
    ],
  },
  {
    id: 'abdominals_thighs',
    name: 'アブドミナル＆サイ',
    emoji: '🔥',
    hint: '腹筋のキレと大腿四頭筋を見せる',
    displays: [
      { muscle: 'abs', weight: 1.0 },
      { muscle: 'obliques', weight: 0.6 },
      { muscle: 'transversus_abdominis', weight: 0.4 },
      { muscle: 'iliopsoas', weight: 0.3 },
      { muscle: 'legs', weight: 0.7 },
      { muscle: 'adductors', weight: 0.3 },
    ],
  },
  {
    id: 'most_muscular',
    name: 'モスト・マスキュラー',
    emoji: '👹',
    hint: '全身を総動員して力強さを見せる',
    displays: [
      { muscle: 'trapezius', weight: 0.7 },
      { muscle: 'chest', weight: 0.7 },
      { muscle: 'shoulder', weight: 0.6 },
...
const CONTESTS: Contest[] = [
  ...
  },
  {
    ...
    name: '全国大会',
    emoji: '🥇',
    flavor: '全7ポーズで全身くまなく審査される。弱点は許されない。',
    poses: CONTEST_POSES.map(p => p.id),
    passLine: 185,
    winLine: 305,
    clearTitleId: 'clear_zenkoku',
    requires: 'ken',
    rivals: [
      { name: '全国区・剛田', emoji: '🦁', comment: '全国のレベル、教えてやる。', baseScore: 180 },
      { name: '前年王者・氷室', emoji: '❄️', comment: '王座は譲らない。完璧に仕上げてきた。', baseScore: 300 },
    ],
  },
  {
    id: 'sekai',
    name: '世界選手権',
    emoji: '🏆',
    flavor: '世界の頂点。完璧な筋量・仕上がり・バランスだけが優勝を許される。',
    poses: CONTEST_POSES.map(p => p.id),
    passLine: 280,
    winLine: 455,
    ...
    requires: 'zenkoku',
    rivals: [
      { name: '世界ランカー・イワン', emoji: '🐻', comment: 'ウォッカより濃い筋肉、見せてやる。', baseScore: 275 },
      { name: '絶対王者・アーノルド', emoji: '👑', comment: 'カムバック。私が世界最強だ。', baseScore: 450 },
    ],
  },
];

function getContest(id: string): Contest {
...
interface ContestState {
  ...
  won: string[];                      // 優勝した大会ID
  bestScores: Record<string, number>; // 大会IDごとのベストスコア
}

const INITIAL_CONTEST_STATE: ContestState = { cleared: [], won: [], bestScores: {} };

// 部位1つの「仕上がり係数」。調子（0.7〜1.3）に、鍛えどき＝キレ増し／オーバーワーク＝キレ減を掛ける。
function getMuscleFinish(mStats: MuscleStats, muscle: MuscleType, now: number): number {
  const tier = getConditionTier(mStats.condition ?? DEFAULT_CONDITION);
  let f = tier.multiplier;
  if (isMuscleSuperCompNow(mStats, muscle, now)) f *= 1.1;       // 鍛えどき＝キレMAX
  else if (isMuscleRecoveringNow(mStats, muscle, now)) f *= 0.9; // オーバーワーク＝張りが出ない
  return f;
}

// 1ポーズの得点。Σ(レベル×重み)＝筋量、見せる部位の仕上がり加重平均＝キレ、タイミング＝ポージング。
function scorePose(pose: PoseDef, stats: AppState, now: number, timingMult: number): number {
  let levelSum = 0;
  let finishWeighted = 0;
  let weightSum = 0;
  for (const d of pose.displays) {
    const m = stats[d.muscle];
    if (!m) continue;
    levelSum += m.level * d.weight;
    finishWeighted += getMuscleFinish(m, d.muscle, now) * d.weight;
    weightSum += d.weight;
  }
  const finishFactor = weightSum > 0 ? finishWeighted / weightSum : 1;
  return Math.round(levelSum * finishFactor * timingMult);
}

// バランス（プロポーション）係数。5部位グループの平均レベルの均整から算出（ゆるめ：0.85〜1.1）。
// 弱点グループがあると下がる＝全身をまんべんなく鍛えるほど高得点。
function getBalanceInfo(stats: AppState): { factor: number; label: string; color: string } {
  const groupAvgs = MUSCLE_GROUPS.map(g => {
    const levels = g.muscles.map(m => stats[m]?.level ?? 1);
  ...
  return (
    <div className="glass-panel" style={{ marginTop: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
        <span>{contest.emoji} {contest.name}</span>
        <span>ポーズ {poseIndex + 1} / {poses.length}　合計 <b style={{ color: 'var(--text-accent)' }}>{runningTotal}</b></span>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', justifyContent: 'center', marginBottom: '1rem' }}>
        {currentPose.displays.map(d => {
          const m = stats[d.muscle];
          const lv = m?.level ?? 1;
          const phaseN = getEvolutionPhase(lv);
          const superComp = m ? isMuscleSuperCompNow(m, d.muscle, Date.now()) : false;
          return (
...
// 大会の「入賞（クリア）称号」の表示名を ACHIEVEMENTS から引く。
function getPlacementTitle(contest: Contest): string {
  return ACHIEVEMENTS.find(a => a.id === contest.clearTitleId)?.name ?? '入賞';
}
...
function App() {
  ...
  // 大会の永続データ。旧セーブには無いので INITIAL_CONTEST_STATE で埋める（後方互換）。
  const [contestState, setContestState] = useState<ContestState>(() => {
    const saved = localStorage.getItem('contestState');
    if (saved) {
      const parsed = JSON.parse(saved);
      return {
        cleared: Array.isArray(parsed.cleared) ? parsed.cleared : [],
        won: Array.isArray(parsed.won) ? parsed.won : [],
  ...
  // 大会画面の遷移。'menu'=大会選択 / 'compete'=ポージング中。
  const [contestScreen, setContestScreen] = useState<'menu' | 'compete'>('menu');
  const [contestId, setContestId] = useState<string>(CONTESTS[0].id);
  ...
  useEffect(() => {
    localStorage.setItem('contestState', JSON.stringify(contestState));
  }, [contestState]);
  ...
  // ===== 大会：バランス評価・ハンドラ・画面描画 =====
  // 全身のバランス（プロポーション）評価。採点にもメニュー表示にも使う。
  const contestBalance = getBalanceInfo(stats);

  const openContest = (id: string) => {
    setContestId(id);
    setContestKey(k => k + 1);
    setContestScreen('compete');
  };

  // 大会終了時：ベストスコアを更新し、入賞なら cleared・優勝なら won に記録して称号を付与（EXPは配らない）。
  const handleContestFinish = (id: string, total: number, placement: 'win' | 'pass' | 'fail') => {
    const contest = getContest(id);
    setContestState(prev => {
      const bestScores = { ...prev.bestScores };
      if (total > (bestScores[id] ?? 0)) bestScores[id] = total;
      const cleared = (placement === 'pass' || placement === 'win') && !prev.cleared.includes(id)
        ? [...prev.cleared, id] : prev.cleared;
      const won = placement === 'win' && !prev.won.includes(id)
        ? [...prev.won, id] : prev.won;
      return { cleared, won, bestScores };
    });
    // 称号付与（入賞で入賞称号、優勝でさらに特別称号）
    const newTitles: string[] = [];
    if (placement === 'pass' || placement === 'win') newTitles.push(contest.clearTitleId);
    if (placement === 'win' && contest.winTitleId) newTitles.push(contest.winTitleId);
    if (newTitles.length > 0) {
      setUnlockedAchievements(prev => {
        const add = newTitles.filter(t => !prev.includes(t));
  ...
  const renderContest = () => {
    if (contestScreen === 'compete') {
      const contest = getContest(contestId);
      const poses = contest.poses.map(pid => CONTEST_POSES.find(p => p.id === pid)!).filter(Boolean);
      return (
        <ContestView
          key={contestKey}
          contest={contest}
    ...
    return (
      <div className="glass-panel" style={{ marginTop: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          {CONTESTS.map(c => {
            const cleared = contestState.cleared.includes(c.id);
            const won = contestState.won.includes(c.id);
            const best = contestState.bestScores[c.id];
            const locked = c.requires ? !contestState.cleared.includes(c.requires) : false;
            return (
              <div key={c.id} className="muscle-card" style={{ padding: '0.8rem', opacity: locked ? 0.55 : 1 }}>
                ...
                {locked ? (
                  <div style={{ textAlign: 'center', fontSize: '0.75rem', color: 'var(--text-secondary)', padding: '0.4rem' }}>
                    🔒 「{getContest(c.requires!).name}」に入賞すると解放
                  </div>
                ) : (
                  <button onClick={() => openContest(c.id)} style={{ width: '100%' }}>{cleared ? 'もう一度出場する' : '出場する'}</button>
                )}
              </div>
            );
          })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
              ...
              {selectedExercise && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '0.5rem' }}>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>使う器具</span>
                  <EquipmentBadge equipment={selectedExercise.equipment} />
                </div>
              )}
